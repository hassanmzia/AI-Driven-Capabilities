...
import { executeSelfConsistency } from '../services/api';
import { ExecutionResult } from '../types';
import { FormattedOutput } from '../components/shared/FormattedOutput';

export function SelfConsistencyVoter() {
  ...
  const handleExecute = async () => {
    ...
    setResult(null);
    try {
      const data = await executeSelfConsistency({ question, num_paths: numPaths, model });
      setResult(data);
    } catch (err: any) {
    ...
    <div style={{ padding: '1rem', color: 'var(--text-primary)' }}>
      <div className="page-header">
        ...
        <p style={{ color: 'var(--text-secondary)' }}>
          Generate multiple independent reasoning paths and vote on the most consistent answer
        </p>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
        <div className="form-group" style={{ marginBottom: '1rem' }}>
          <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Question / Task</label>
          <textarea
            className="form-textarea"
            value={question}
            onChange={(e) => setQuestion(e.target.value)}
            placeholder="Enter a question or task that benefits from multiple reasoning approaches..."
            rows={4}
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <div className="form-group" style={{ flex: 1, minWidth: '150px' }}>
            <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Reasoning Paths</label>
            <input
        ...
        <button className="btn btn-primary" onClick={handleExecute} disabled={loading || !question.trim()}>
          {loading ? 'Analyzing...' : 'Run Self-Consistency Vote'}
        </button>
      </div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '1rem' }}>Generating multiple reasoning paths...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <span className="card-title" style={{ color: 'var(--text-primary)' }}>Self-Consistency Results</span>
          </div>
...
export default SelfConsistencyVoter;
