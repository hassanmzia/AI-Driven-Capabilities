...
import { executeCoordinatorRouter } from '../services/api';
import { ExecutionResult } from '../types';
import { FormattedOutput } from '../components/shared/FormattedOutput';

export function CoordinatorRouter() {
  ...
  const handleExecute = async () => {
    ...
    setResult(null);
    try {
      const agents = agentsInput.split('\n').map(a => a.trim()).filter(Boolean);
      const data = await executeCoordinatorRouter({ task, agents, model });
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: '1rem', color: 'var(--text-primary)' }}>
      <div className="page-header">
        <h1 style={{ color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Coordinator / Router</h1>
        <p style={{ color: 'var(--text-secondary)' }}>
          Intelligently route complex tasks across multiple specialist agents
        </p>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
        <div className="form-group" style={{ marginBottom: '1rem' }}>
          ...
          <textarea className="form-textarea" value={task} onChange={(e) => setTask(e.target.value)}
            placeholder="Describe the complex task to route..." rows={4}
            style={{ width: '100%', color: 'var(--text-primary)', backgroundColor: 'var(--bg-card)' }} />
        </div>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <div className="form-group" style={{ flex: 2, minWidth: '200px' }}>
            <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Available Agents (one per line)</label>
            <textarea className="form-textarea" value={agentsInput} onChange={(e) => setAgentsInput(e.target.value)}
              placeholder="Enter agent names, one per line..." rows={3}
              style={{ width: '100%', color: 'var(--text-primary)', backgroundColor: 'var(--bg-card)' }} />
          </div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '1rem' }}>Analyzing and routing task...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <span className="card-title" style={{ color: 'var(--text-primary)' }}>Routing Plan</span>
          </div>
