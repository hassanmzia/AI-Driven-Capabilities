...
import { executeTreeOfThoughts } from '../services/api';
import { ExecutionResult } from '../types';
import { FormattedOutput } from '../components/shared/FormattedOutput';

export function TreeOfThoughts() {
  ...
  const handleExecute = async () => {
    ...
    setResult(null);
    try {
      const data = await executeTreeOfThoughts({ problem, num_branches: numBranches, max_depth: maxDepth, model });
      setResult(data);
    } catch (err: any) {
    ...
    <div style={{ padding: '1rem', color: 'var(--text-primary)' }}>
      <div className="page-header">
        ...
        <p style={{ color: 'var(--text-secondary)' }}>
          Explore multiple reasoning branches in a tree structure to find optimal solutions
        </p>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
        <div className="form-group" style={{ marginBottom: '1rem' }}>
          <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Problem</label>
          <textarea
            className="form-textarea"
            value={problem}
            onChange={(e) => setProblem(e.target.value)}
            placeholder="Describe a problem that benefits from exploring multiple reasoning branches..."
            rows={5}
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <div className="form-group" style={{ flex: 1, minWidth: '150px' }}>
            <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Branches</label>
            <input
          ...
          <div className="form-group" style={{ flex: 1, minWidth: '150px' }}>
            <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Max Depth</label>
            <input
              className="form-input"
              type="number"
              min={2}
        ...
        <button className="btn btn-primary" onClick={handleExecute} disabled={loading || !problem.trim()}>
          {loading ? 'Exploring...' : 'Explore Thought Tree'}
        </button>
      </div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '1rem' }}>Exploring reasoning branches...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <span className="card-title" style={{ color: 'var(--text-primary)' }}>Tree of Thoughts Results</span>
          </div>
