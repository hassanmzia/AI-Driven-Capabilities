...
import { executeSelfVerification } from '../services/api';
import { ExecutionResult } from '../types';
import { FormattedOutput } from '../components/shared/FormattedOutput';

export function SelfVerificationChain() {
  ...
  const handleExecute = async () => {
    if (!task.trim() || !inputText.trim()) return;
    setLoading(true);
    setError(null);
    setResult(null);
    try {
      const data = await executeSelfVerification({ task, input_text: inputText, model });
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: '1rem', color: 'var(--text-primary)' }}>
      <div className="page-header">
        <h1 style={{ color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Self-Verification Chain</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Generate responses with built-in verification checks to validate correctness</p>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
        <div style={{ marginBottom: '1rem' }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Task Description</label>
          <textarea
            value={task}
        ...
        <div style={{ marginBottom: '1rem' }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Input</label>
          <textarea
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
            rows={4}
            placeholder="Provide the input for the task..."
            style={{ width: '100%', padding: '0.75rem', backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '0.375rem', resize: 'vertical', fontFamily: 'inherit', fontSize: '0.85rem' }}
          />
        </div>
        ...
        <button className="btn btn-primary" onClick={handleExecute} disabled={loading || !task.trim() || !inputText.trim()}>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '1rem' }}>Generating and verifying response...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <span className="card-title" style={{ color: 'var(--text-primary)' }}>Verified Response</span>
          </div>
...
export default SelfVerificationChain;
