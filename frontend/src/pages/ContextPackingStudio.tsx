...
import { executeContextPacker } from '../services/api';
import { ExecutionResult } from '../types';
import { FormattedOutput } from '../components/shared/FormattedOutput';

export function ContextPackingStudio() {
  ...
  const handleExecute = async () => {
    ...
    setResult(null);
    try {
      const data = await executeContextPacker({ content, token_budget: tokenBudget, model });
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: '1rem', color: 'var(--text-primary)' }}>
      <div className="page-header">
        <h1 style={{ color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Context Packing Studio</h1>
        <p style={{ color: 'var(--text-secondary)' }}>Optimize content for maximum information density within token budgets</p>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
        <div style={{ marginBottom: '1rem' }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Content to Optimize</label>
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            rows={6}
            placeholder="Paste the content you want to compress and optimize..."
        ...
        <div style={{ marginBottom: '1rem' }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Token Budget</label>
          <input
            type="number"
            value={tokenBudget}
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '1rem' }}>Analyzing and optimizing content...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <span className="card-title" style={{ color: 'var(--text-primary)' }}>Optimization Results</span>
          </div>
...
export default ContextPackingStudio;
