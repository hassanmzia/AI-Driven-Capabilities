...
import { executeReActAgent } from '../services/api';
import { ExecutionResult } from '../types';
import { FormattedOutput } from '../components/shared/FormattedOutput';

export function ReActAgent() {
  const [task, setTask] = useState('');
  const [toolsInput, setToolsInput] = useState('search, calculate, lookup, analyze, compare');
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      const data = await executeReActAgent({ task, available_tools: tools, model });
      setResult(data);
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: '1rem', color: 'var(--text-primary)' }}>
      <div className="page-header">
        <h1 style={{ color: 'var(--text-primary)', marginBottom: '0.5rem' }}>ReAct Agent Builder</h1>
        <p style={{ color: 'var(--text-secondary)' }}>
          Simulate a ReAct agent using Thought-Action-Observation reasoning loops
        </p>
      </div>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
        <div className="form-group" style={{ marginBottom: '1rem' }}>
          ...
          <textarea className="form-textarea" value={task} onChange={(e) => setTask(e.target.value)}
            placeholder="Describe the task for the ReAct agent to solve..." rows={4}
            style={{ width: '100%', color: 'var(--text-primary)', backgroundColor: 'var(--bg-card)' }} />
        </div>
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <div className="form-group" style={{ flex: 2, minWidth: '200px' }}>
            <label className="form-label" style={{ color: 'var(--text-secondary)' }}>Available Tools (comma-separated)</label>
            <input className="form-input" type="text" value={toolsInput} onChange={(e) => setToolsInput(e.target.value)}
        ...
        <button className="btn btn-primary" onClick={handleExecute} disabled={loading || !task.trim()}>
          {loading ? 'Running Agent...' : 'Run ReAct Agent'}
        </button>
      </div>
      ...
      {loading && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '1rem' }}>Agent reasoning in progress...</p>
        </div>
      )}
      ...
      {result && !loading && (
        <div className="card" style={{ padding: '1.5rem', backgroundColor: 'var(--bg-card)' }}>
          <div className="card-header" style={{ marginBottom: '1rem' }}>
            <span className="card-title" style={{ color: 'var(--text-primary)' }}>ReAct Agent Results</span>
          </div>
